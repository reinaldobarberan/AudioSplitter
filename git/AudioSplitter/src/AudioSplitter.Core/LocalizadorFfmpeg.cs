...
public static class LocalizadorFfmpeg
{
    ...
    /// <summary>Cuántas rutas se listan en el error antes de resumir el resto.</summary>
    private const int RutasEnElError = 8;

    /// <summary>
    /// Rutas completas donde se busca el ejecutable, en orden y sin repetidos.
    /// Es público para poder diagnosticar una instalación sin tener que provocar el error.
    /// </summary>
    public static IReadOnlyList<string> RutasCandidatas(string nombre)
    {
        var exe = OperatingSystem.IsWindows() ? nombre + ".exe" : nombre;

        // El PATH del proceso y el del registro se solapan casi por completo, así que sin
        // deduplicar el mensaje de error repite las mismas carpetas tres o cuatro veces.
        var comparador = OperatingSystem.IsWindows()
        ...
        foreach (var carpeta in CarpetasCandidatas())
        {
            ...
            if (vistos.Add(candidato)) rutas.Add(candidato);
        }
    ...
    {
        var rutas = RutasCandidatas(nombre);

        foreach (var candidato in rutas)
            if (File.Exists(candidato)) return candidato;

        var exe = OperatingSystem.IsWindows() ? nombre + ".exe" : nombre;
        throw new FileNotFoundException(Explicar(exe, rutas), exe);
    }

    /// <summary>
    /// Un error accionable: primero qué hacer, después dónde se buscó. La lista va recortada
    /// porque el PATH de una máquina de trabajo puede tener decenas de entradas, y un muro
    /// de texto no ayuda a nadie.
    /// </summary>
    internal static string Explicar(string exe, IReadOnlyList<string> buscados)
    {
        var carpetaApp = AppContext.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar);

        var mensaje =
            $"No se encontró {exe}.\n\n" +
            "Para solucionarlo, cualquiera de estas dos:\n" +
            $"  1. Copiá ffmpeg.exe y ffprobe.exe a:\n     {carpetaApp}\n" +
            "  2. Instalá FFmpeg y agregá su carpeta al PATH del sistema.\n\n" +
            $"Se buscó en {buscados.Count} ubicaciones";

        if (buscados.Count <= RutasEnElError)
            return mensaje + ":\n" + string.Join("\n", buscados.Select(b => "  · " + b));

        return mensaje + $", entre ellas:\n" +
               string.Join("\n", buscados.Take(RutasEnElError).Select(b => "  · " + b)) +
               $"\n  … y {buscados.Count - RutasEnElError} más del PATH.";
    }
